...
t = pd.read_csv("other disease/Training.csv")
tr = pd.read_csv("other disease/Testing.csv")
...
# THE DUPLICATE FREE DATASET IS WRITTEN INTO CLEANTRAINING.CSV
# NOW LOADING CLEANTRAINING.CSV FOR TRAINING

df = pd.read_csv("other disease/cleantraining.csv")

# DATA PREPROCESSING

# CONVERTING STRING TO INTEGERS IN THE DATASET
...
tr.replace(
    {'prognosis': {'Fungal infection': 0, 'Allergy': 1, 'GERD': 2, 'Chronic cholestasis': 3, 'Drug Reaction': 4,
                   ...
                   'Jaundice': 14, 'Malaria': 15, 'Chicken pox': 16,
                   'Dengue': 17, 'Typhoid': 18, 'hepatitis A': 19, 'Hepatitis B': 20, 'Hepatitis C': 21,
                   'Hepatitis D': 22, 'Hepatitis E': 23,
...
y_test = tr['prognosis'].values.tolist()
x_train = df.drop('prognosis', axis=1)
y_train = df['prognosis'].values.tolist()
...
accuracy_score(y_test, y_pred)  # 0.95
...
accuracy_score(y_test, ynb)  # 0.97
...
accuracy_score(y_test, ysvm)  # 0.93
...
accuracy_score(y_test, ydtc)  # 0.17
...
accuracy_score(y_test, ypre)  # 1


def remdis(clicked1, clicked2, clicked3, clicked4, clicked5):

    l2 = []
    for x in range(0, len(l1)):
        l2.append(0)

    psymptoms = [clicked1, clicked2, clicked3, clicked4, clicked5]
    ...
    inputtest = [l2]
    predict = clf.predict([l2])
    predicted = predict[0].astype(np.int_)
    output = disease[predicted]
    return output
